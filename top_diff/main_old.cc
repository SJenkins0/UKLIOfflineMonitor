...
int main(int argc, char *argv[]){
  ...
  //
  double timeLowTot, timeHighTot, timeLowSpt, timeHighSpt;
  if(run >= 87073){
    timeLowTot = 650;
    timeHighTot = 1250;
    timeLowSpt = 700;
  ...
  for (Int_t evnt =0; evnt < nEvnt; ++evnt){
	...
	if (pmtz_vec->at(count) < 0 && pmty_vec->at(count) > 0){
	  bzlt0y0 += charge_vec->at(count);
	}
	if (pmtz_vec->at(count) < 0 && pmty_vec->at(count) < 0){
	  bzylt0 += charge_vec->at(count);
	}
	if (pmtz_vec->at(count) < 0 && pmtz_vec->at(count) > -1795 && pmty_vec->at(count) < 0){
      ...
      //dark noise
      if(time_vec->at(count) > 500 && time_vec->at(count) < 650){
    ...
    //Add in monitor charge
    
    dataFile << run << " " << subrun << " " << year << " " << month << " " << day << " "
	     << hour << " " << minute << " " << second << " " << totQ << " "
	     << barrelQ << " " << bz0y0 << " " << bottomQ << " " << bzlt0y0 << " " << bzylt0 << " " << barzylt0 << " " << noiseQ << "\n";
