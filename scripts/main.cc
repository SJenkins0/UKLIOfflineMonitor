  ...
  for (Int_t evnt =0; evnt < nEvnt; ++evnt){
	...
	if (timeCor < tofCutVal){
	  ...
	  if (theta < opAng){
	    ...
	    /*
	    //Add new diffuser info
	    if(beamType=="diffuser"){
	      if(pmtz_vec->at(count) < tar.Z()+zDist && pmtz_vec->at(count) > tar.Z()-zDist){
		if(theta<20)
		  Qincut += charge_vec->at(count);
		else
		  Qoutcut += charge_vec->at(count);
	      }
	    }
	    */
	  }
      ...
      if(time_vec->at(count) > 500 && time_vec->at(count) < 1000){
	nNoise += 1;
	noiseQ += charge_vec->at(count);
      }

      //Select event in a 1000 ns window to later remove noise
      if(time_vec->at(count) > noiseLowCut && time_vec->at(count) < noiseHighCut){
